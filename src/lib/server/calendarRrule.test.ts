...
function rem(partial: Partial<Reminder>): Reminder {
	return {
		...
		description: null,
		type: 'medication',
		dueAt: new Date('2026-06-15T13:00:00Z'),
		isRecurring: true,
		recurrenceUnit: null,
...
describe('reminderRecurrence', () => {
	...
	it('daily', () => {
		expect(
			reminderRecurrence(
				rem({ recurrenceUnit: 'day', recurrenceInterval: 2 }),
	...
	it('weekly pins BYDAY from the DTSTART weekday in tz', () => {
		...
		expect(
			reminderRecurrence(
				rem({ recurrenceUnit: 'week', recurrenceInterval: 1 }),
				new Date('2026-06-15T13:00:00Z'),
				TZ
			)
	...
	it('monthly with anchor day < 29 → BYMONTHDAY', () => {
		expect(
			reminderRecurrence(
				rem({ recurrenceUnit: 'month', recurrenceInterval: 1, recurrenceAnchorValue: 15 }),
				new Date('2026-06-15T13:00:00Z'),
				TZ
			)
	...
	it('monthly with anchor day >= 29 → RDATE expansion (clamped cadence)', () => {
		const r = reminderRecurrence(
			rem({ recurrenceUnit: 'month', recurrenceInterval: 1, recurrenceAnchorValue: 31 }),
	...
	it('yearly packs BYMONTH/BYMONTHDAY', () => {
		...
		expect(
			reminderRecurrence(
				rem({ recurrenceUnit: 'year', recurrenceInterval: 1, recurrenceAnchorValue: 620 }),
